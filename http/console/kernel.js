...
class Kernel {
  ...
  async help() {
    let help = [
      ...
      {
        command: "node com migrate table1 table2 ...",
        description: "Migrate table1, table2 ... by sequence.",
      },
      {
      ...
      {
        command: "node com seed table1 table2 ...",
        description: "Seed table1, table2 ... by sequence.",
      },
      {
    ...
    let helpText =
      help
        .map((h) => {
          ...
          return (
            com_colours.fg.green +
            h.command +
            ws +
            com_colours.fg.yellow +
            " " +
            h.description
          )
        })
  ...
  async migrate() {
    ...
    let files = filesOrDirs.filter((fileOrDir) => {
      return (
        fs.statSync(this.migrationPath + "/" + fileOrDir).isFile() &&
        fileOrDir.endsWith(fileExt)
      )
    })
    ...
    for (let fileObj of filesObjToBeUp) {
      try {
        const MyTableClass = require("../../migrations/" +
          fileObj.time +
          "-" +
          fileObj.name)
  ...
  async makeMigration() {
    ...
    for (const table of tables) {
      try {
        ...
        let fileName =
          moment().format("yyyyMMDDHHmmssSSS") + "-" + table + fileExt
  ...
  async seed() {
    ...
    let files = filesOrDirs.filter((fileOrDir) => {
      return (
        fs.statSync(this.seederPath + "/" + fileOrDir).isFile() &&
        fileOrDir.endsWith(fileExt)
      )
    })
    let filesObj = files.map((file) => {
    ...
    for (let fileObj of filesObjToBeUp) {
      try {
        const MyTableClass = require("../../seeders/" +
          fileObj.time +
          "-" +
          fileObj.name)
        await new MyTableClass().up()
  ...
  async launchCommand() {
    ...
    for (let file of files) {
      try {
        ...
        const MyCommandClass = require("./commands/" +
          file.relativePath +
          slash +
          file.file)
  ...
  async _makeAny(
    ...
    for (let arg of argItems) {
      try {
        ...
        className =
          className[0].toUpperCase() + className.slice(1) + classSuffix
        let mf = fs.readFileSync(
          __dirname + "/kernel/" + kernelResource + ".js",
          "utf8"
        )
        let mfArr = mf.split("/*" + kernelResource + "-separator*/")
        let relPath = path
          .relative(
            path.join(itemPath, additionalPath),
            this.root + "/components/db"
          )
          .replace(/\\/gi, "/")
        mfArr[0] =
          'const {DB} = require("' +
        ...
        for (let repl in replacements) {
          mfArr[repl] =
            typeof replacements[repl] === "function"
              ? replacements[repl](arg, dirLessArg, className, fileName)
              : replacements[repl]
        }
...
const nodeCommand = async (command) => {
  let args = command.split(/\s+/gi)
